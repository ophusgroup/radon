...
class RadonForward(Function):
    ...
    def forward(
        ...
        exec_cfg = (
            exec_cfg_generator(vol_cfg, proj_cfg, x.dtype == torch.half)
            if exec_cfg is None
            else exec_cfg
        )
        sinogram = cuda_backend.forward(
            x, angles, tex_cache, vol_cfg, proj_cfg, exec_cfg
        )
    ...
    def backward(ctx, grad_x):
        ...
        exec_cfg = ctx.exec_cfg_generator(
            ctx.vol_cfg,
            ctx.proj_cfg,
            grad_x.dtype == torch.half,
        )
        grad = cuda_backend.backward(
            grad_x,
            angles,
            ctx.tex_cache,
            ctx.vol_cfg,
